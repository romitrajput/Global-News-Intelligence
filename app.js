...
const Engine = (function () {
  ...
  function analyze(raw, opts) {
    ...
    const headline = cleanLine(opts.headline || '') ? clip(cleanLine(opts.headline), 160) : pickHeadline(raw.replace(/\r/g, ''), opts.fallbackTitle);
    const head = (headline + '\n' + text.slice(0, 300));
...
if (typeof document !== 'undefined') (function () {
  ...
  function entryHTML(it) {
    ...
    return `<article class="entry imp-${it.importance.toLowerCase()}${open ? ' open' : ''}" data-id="${it.id}">
      ...
      <div class="foot"><span class="imp"><i></i>${it.importance}</span>${n > 1 ? `<span>${n} sources</span>` : ''}${rel ? `<span>${rel} related</span>` : ''}<span>${ago(it.addedAt)}</span></div>
      ${open ? detailsHTML(it) : ''}
    </article>`;
  }

  function detailsHTML(it) {
    const rel = (it.related || []).map(id => S.items.find(x => x.id === id)).filter(Boolean);
    ...
    return `<div class="details">
      ...
      <dl class="kv">
        ...
        ${row('Sources', (it.sources || []).map(s => esc(s.name)).join(', '))}
      </dl>
  ...
  async function renderExport() {
    ...
    $('#storeInfo').textContent = n + (n === 1 ? ' item is' : ' items are') + ' stored on this phone. Nothing is uploaded anywhere.';
    $$('#exportSeg button').forEach(b => b.setAttribute('aria-pressed', b.dataset.v === S.exportRange));
    const c = S.items.filter(i => inRange(i, S.exportRange)).length;
  ...
  function exportCSV() {
    const items = S.items.filter(i => inRange(i, S.exportRange)).sort(byPriority);
  ...
  $('#clearBtn').addEventListener('click', async () => {
    ...
    if (!confirm('Delete all ' + S.items.length + ' items from this phone? This cannot be undone.')) return;
  ...
  (async function start() {
    ...
    setTab('inbox'); renderFiles(); renderAll();
